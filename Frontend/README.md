...
Il progetto è diviso in due parti che comunicano tra loro: un **frontend** realizzato in Angular, che gestisce l'interfaccia con cui l'utente interagisce, e un **backend** in Node.js, che si occupa della logica dell'applicazione e del salvataggio dei dati su un database MySQL.
...
È necessario avere un database MySQL avviato in locale e configurato secondo il file `database.sql` presente nella cartella `Backend`.
